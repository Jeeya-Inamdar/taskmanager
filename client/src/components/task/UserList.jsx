...
const UserList = ({ setTeam, team }) => {
  ...
  // Function to update the selected users and team state
  const handleChange = (selected) => {
    setSelectedUsers(selected);
    setTeam(selected.map((user) => user._id)); // Store only user IDs in the team state
  };
  ...
  useEffect(() => {
    if (!isLoading && data) {
      if (team?.length > 0) {
        ...
        setSelectedUsers([data[0]]); // Default selection
      }
    }
  }, [isLoading, data, team]);
